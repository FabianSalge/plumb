...
(pinned revision, no remote code), the vendored prompt format it must
reproduce, how token-level hallucination probabilities reduce to per-passage
support scores behind the `Scorer` interface, span detail for observability,
and fail-loud behavior on malformed model output.
