...
# Model weights are downloaded at first start, not baked into the image: HHEM
# is ~420 MB, its revision is pinned in config/verifier.yaml, and /readyz
# already reports "loading model" until the load completes. Baking would
# couple image rebuilds to weights that change independently of the code and
# triple push/pull times. Revisit when vLLM lands.
...
RUN --mount=type=cache,target=/root/.cache/uv \
    ...
    uv sync --frozen --no-dev --no-install-project --extra hhem
...
RUN --mount=type=cache,target=/root/.cache/uv \
    uv sync --frozen --no-dev --no-editable --extra hhem
